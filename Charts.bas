...
Sub AddBarPoint (BD As BarData, X As String, YArray() As Float) 'ignore
...
Sub AddBarColor(BD As BarData, Color As Int) 'ignore
	If BD.BarsColors.IsInitialized = False Then BD.BarsColors.Initialize
	BD.BarsColors.Add(Color)
End Sub

Sub DrawBarsChart(G As Graph, BD As BarData, BackColor As Int) 'ignore
...
Sub AddLineMultiplePoints(LD As LineData, X As String, YArray() As Float, ShowTick As Boolean) 'ignore
	If LD.Points.IsInitialized = False Then LD.Points.Initialize
	Dim p As LinePoint
	p.Initialize
	p.X = X
...
Sub AddPieItem(PD As PieData, Name As String, Value As Float, Color As Int) 'ignore
...
Sub DrawPie (PD As PieData, BackColor As Int, CreateLegendBitmap As Boolean) As Bitmap 'ignore
